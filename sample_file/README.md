...
| 檔案 | 說明 |
|---|---|
| `AllCueList.xlsx` | 範例燈表：前 4 張是 `說明` / `(勿動)下拉選單` / `空白模板` / 測試用，第 5 張起放各組燈流，每組一張工作表、每張兩個自動化表格 |
...
第 1 步預設從第 5 張工作表開始，所以 `說明`、`(勿動)下拉選單`、`空白模板` 與第 4 張的測試用表
不會被轉換。第 5 張起每張工作表會變成一個 Show 與兩條 Sequence（60RC 與 ER554 各一），
